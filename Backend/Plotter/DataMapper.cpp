...
DataMapper::Point DataMapper::mapDataToScreen(double x, double y, const AxisRange& xRange, const AxisRange& yRange) {
    ...
    // Check for invalid ranges (protect against division by zero)
    if (xRange.max <= xRange.min || yRange.max <= yRange.min) {
        printf("Warning: Invalid axis range detected in mapDataToScreen\n");
        printf("  X range: [%.2f, %.2f], Y range: [%.2f, %.2f]\n", 
               xRange.min, xRange.max, yRange.min, yRange.max);
        
        // Return center point as fallback
        int centerX = layout.getMarginLeft() + plotWidth / 2;
        int centerY = layout.getMarginTop() + plotHeight / 2;
    ...
    // Ensure minimum span to prevent division by zero
    if (xSpan < 1e-10) xSpan = 1.0;
    if (ySpan < 1e-10) ySpan = 1.0;
    
    // Map X coordinate from data space to screen space (clamped to range)
    double xValue = x;
    if (xValue < xRange.min) xValue = xRange.min;
    if (xValue > xRange.max) xValue = xRange.max;
    
    double xRatio = (xValue - xRange.min) / xSpan;
    ...
    double yValue = y;
    if (yValue < yRange.min) yValue = yRange.min;
    if (yValue > yRange.max) yValue = yRange.max;
    
    double yRatio = (yValue - yRange.min) / ySpan;
    int screenY = layout.getHeight() - layout.getMarginBottom() - static_cast<int>(yRatio * plotHeight);
    
    // Ensure the point is within the plot bounds (double-check bounds)

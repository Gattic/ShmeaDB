    ...
    const DataMapper::AxisRange& yRange)
{
    ...
    for (size_t i = 0; i < data.size(); ++i) {
        if (labels[i] == cluster && data[i].size() >= 2) {
            ...
            DataMapper::Point p = dataMapper.mapDataToScreen(data[i][0], data[i][1], xRange, yRange);
    ...
    if (count > 0) {
        ...
        int centerX = static_cast<int>(sumX / count);
        int centerY = static_cast<int>(sumY / count);
    ...
    const DataMapper::AxisRange& yRange)
{
    ...
    for (size_t i = 0; i < centroids.size() && i < clusterColors.size(); ++i) {
        ...
        DataMapper::Point p = dataMapper.mapDataToScreen(
            centroids[i][0], centroids[i][1], xRange, yRange);
        ...
        // Draw the white circle on top of the shadow - exactly as in plotter.cpp
        RGBA whiteFill(0xFF, 0xFF, 0xFF, 0xFF); // Solid white
